...
import com.praktikum.print.KembaliPrint;
import com.praktikum.print.model.KembaliPM;
import com.praktikum.table.model.KembaliTreeTableNodeModel;
...
public class KembaliTree extends TreeTableDefault  {
	...
	public void actionPrint(){
		KembaliPrint p=new KembaliPrint(panel);
		actionSearch();
		List<KembaliPM> pm=new ArrayList<KembaliPM>();
		for (Object o : models) {
			if (o instanceof Kembali) {
				Kembali s=(Kembali) o;
				KembaliPM tmp1=new KembaliPM(s);
				pm.add(tmp1);
				List<Kembalid> sewads=App.getKembalidDao().getAllByKembali(s.getKembaliId());
				int tmp=1;
				for (Kembalid sewad : sewads) {
					sewad.setNumber(tmp);
					sewad.setKembali(s);
					KembaliPM tmp2=new KembaliPM(sewad);
					pm.add(tmp2);
					tmp++;
				}
			}
	...
	public void actionSearch(){
		reloadSearchLap();
		root = new DefaultMutableTreeTableNode(LaporanM.TITLE_MENU);
		for (Object o : models) {
			KembaliTreeTableNodeModel p = new KembaliTreeTableNodeModel(o);
			root.add(p);
		}
...
//			showColumn.setText("Show All");
	...
	public SplitButton getItemSearch() {
		if (itemSearch == null) {
			...
			JMenuItem item = new JMenuItem(LWindow.KET_SEARCH + LKembali.CODE);
			item.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
					itemSearch.setText(LWindow.KET_SEARCH + LKembali.CODE);
					colSearch = "code";
				}
			});
			menuItemSearch.add(item);

			item = new JMenuItem(LWindow.KET_SEARCH + LKembali.TGL_TRX);
			item.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
	...
	public MutableTreeTableNode createNode(Kembali o) {
		return new KembaliTreeTableNodeModel(o);
	}
...
//						KembaliTreeTableNodeModel x=(KembaliTreeTableNodeModel)nodeSelected;
//						Object tmpo=x.getUserObject();
//						if (tmpo instanceof Kembali) {
//							Kembali tmp=(Kembali) x.getUserObject();
//						}
...
//		PaketTreeTableNodeModel n=new PaketTreeTableNodeModel(o);
		//root.add(n);
//		if (o.field("@class").equals(Kembali.TABLE)) {
	...
	public void aksiDelete() {
		...
		if (tmp!=null) {
			if (JOptionPane.showConfirmDialog(null,
				...
				}else if (tmp instanceof Kembali) {
					App.getKembaliDao().delete((Kembali) tmp);
				}
			}
...
//				}
	...
	public void loadDataModelSearchLap(Date tglStart, Date tglEnd, Pelanggan modelPelanggan, Pegawai modelPegawai, TypeMobil modelTypeMobil, Mobil modelMobil) {
			models = App.getKembaliDao().getAllByColumnLaporan(tglStart, tglEnd, modelPelanggan, modelPegawai, modelTypeMobil, modelMobil);
	}
